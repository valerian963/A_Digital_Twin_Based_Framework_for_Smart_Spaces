...
class Server():
  def __init__(self,port,pathComplement,pswd):
     self.end,self.userName,self.path,self.port = ip,'valeria','/root/config/sensores/data/'+pathComplement,port
     self.pswd = senha1 if pswd == '' else pswd

  def get(self): return [self.end,self.userName,self.pswd,self.port,self.path]

server1,server2,server3,server4 = Server(102,'server1/Server1_Data.xml',''),Server(112,'server2/Server2_Data.xml',''),Server(122,'server3/Server3_Data.xml',''),Server(132,'server4/Server4_Data.xml',senha2)
